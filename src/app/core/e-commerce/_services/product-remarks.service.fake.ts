...
export class ProductRemarksService {
  ...
  getProductRemarkById(productRemarkId: string): Observable<ProductRemarkModel> {
  ...
  deleteProductRemark(productRemarkId: string): Observable<ProductRemarkModel> {
    const url = `${API_PRODUCTREMARKS_URL}/${productRemarkId}`;
    return this.http.delete<ProductRemarkModel>(url);
  }

  deleteProductRemarks(ids: string[] = []): Observable<any> {
    const tasks$ = [];
